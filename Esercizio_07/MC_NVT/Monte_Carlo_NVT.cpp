...
int main() { 
	...
	Equilibrate(5000);	//Equilibration

	cout << "Equilibration done. The simulation starts" << endl << endl;
...
// distance in the histogram then increase
			bin = dr/bin_size;
...
void Averages(int iblk) { //Print results for current block
	...
	Epot.open("epot.0",ios::app);
	Pres.open("pres.0",ios::app);
	Gofr.open("gofr.0",ios::app);
	Gave.open("gave.0",ios::app);
...
// print (r g(r) for each block and final from last block with error)
		Gofr << r << "    " << stima_gofr << endl;
		if( iblk == nblk )
			Gave << r << "    " << glob_av[i]/(double)iblk << "    " << err_gdir << endl;
	}
...
void Instant() {	//Print instant values of observables
	ofstream Epot, Pres, Gofr;

	Epot.open("epot.ist",ios::app);
    Pres.open("pres.ist",ios::app);

	Epot << walker[iv]/(double)npart + vtail << endl;	//energy per particle
	Pres << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;	//pression
