...
//Prepare initial velocities
		...
		for (int i=0; i<npart; ++i) {
			...
			xold[i] = x[i] - vx[i] * delta;	//dalle posizioni attuali a
			yold[i] = y[i] - vy[i] * delta;	//quelle precedenti
...
//Kinetic energy
	...
	stima_pres = rho * stima_temp + (w/3. + ptail * (double)npart)/vol;
