...
// Optimezed parameters
	/*double mu = 0.82;
	double sigma = 0.6 * 0.6;
	
	double n1 = exp( -pow(v-mu, 2)/(2. * sigma) );
	double n2 = exp( -pow(v+mu, 2)/(2. * sigma) );
...
double variationalWaveFunction_second(double v)
{
	...
	double e1 = pow(v-mu, 2)/sigma;
	double e2 = pow(v+mu, 2)/sigma;

	return ( -exp(-e1/2.) - exp(-e2/2.) + e1 * exp(-e1/2.) + e2 * exp(-e2/2.) )/sigma;*/
	return 0;
